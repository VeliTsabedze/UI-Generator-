...
describe("ChatContext", () => {
  ...
  test("passes through AI chat functionality", () => {
    ...
    fireEvent.change(textarea, { target: { value: "Build me a button" } });
    expect(textarea.value).toBe("Build me a button");

    fireEvent.submit(form);

    expect(mockSendMessage).toHaveBeenCalledWith({ text: "Build me a button" });
  });
  ...
  test("handles tool calls", () => {
    ...
    const toolCall = { toolName: "test", input: { foo: 1 } };
